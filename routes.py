    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        with con:
            ...
            if lginfo:
                ...
                session['id'] = lginfo[0]
                c.execute("SELECT * from EMPLOYEES WHERE EMPLOYEE_ID = ?",(lginfo[0]))
                empinfo = c.fetchone()
    ...
    if request.method == 'POST' and 'first_name' in request.form and 'last_name' in request.form and 'email' in request.form and 'authority' in request.form:
        ...
        with con:
            ...
            #    msg = 'Invalid email address!'
                ...
                c.execute("INSERT INTO LOGIN_INFO (EMPLOYEE_ID, USERNAME, EMAIL_ID, AUTHORITY) VALUES (?, ?, ?, ?)",(int(uid), rusername, remail, authority))
                con.commit()
                return redirect(url_for('index'))
...
def evacreq():
    ...
    if request.method == 'POST' and 'start_date' in request.form and 'end_date' in request.form:
        ...
        with con:
                c = con.cursor()
                c.execute("INSERT INTO VACATION_REQ (EMPLOYEE_ID, LASTNAME, FIRSTNAME, EMP_POSITION, START_DATE, END_DATE, TOTAL_DAYS) VALUES (?,?,?,?,?,?,?)" ,(empid, lname, fname, emposition, stdate, eddate, totdays))
                con.commit()
                c.execute("SELECT * from VACATION_REQ WHERE EMPLOYEE_ID = ?",(empid))
                evacreqinfo = c.fetchall()
...
@app.route('/profile/')
def profile():
    return render_template('profile.html', flname=flname, first_name=fname, last_name=lname, emailid = emailid, user_name=uname)